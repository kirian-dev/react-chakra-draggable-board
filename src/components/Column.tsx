...
import useColumnDrop from '../common/hooks/useColumnDrop';
import useColumnTasks from '../common/hooks/useColumnTasks';
...
const Column: FC<{ column: ColumnType }> = ({ column }) => {
  ...
  const ColumnTasks = tasks?.map((task, index) => (
    <Task key={task.id} task={task} index={index} onDropHover={swapTasks} onUpdate={updateTask} onDelete={deleteTask} />
  ));
